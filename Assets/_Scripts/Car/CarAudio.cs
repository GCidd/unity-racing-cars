...
public class CarAudio : MonoBehaviour
{
    ...
    {
        GameObject player = GameObject.Find("Player Car");
        float factor = 10 / (Vector3.Distance(player.transform.position, transform.position));
        return 1 * Mathf.Min(1.5f, factor);
    }
    ...
    private void Update()
    {
        ...
        if (m_StartedSound)
        {
            ...
            else
            {
                ...
                m_LowAccel.volume = lowFade*accFade * GetVolume();
                m_LowDecel.volume = lowFade*decFade * GetVolume();
                m_HighAccel.volume = highFade*accFade * GetVolume();
                m_HighDecel.volume = highFade*decFade * GetVolume();
